        ...
        <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "10px" }}>
                {groceries.map((grocery) => (
                    <div key={grocery.id} >
                        <img src={grocery.image} alt={grocery.name} style={{ width: "50%" ,height:"50%"}} />
                        <p>price {grocery.price}</p>
                        <p>Name: {grocery.name}</p>
                        <button  onClick={()=>setpage(page-1)} style={{backgroundColor:"black",color:"white",margin:"10px 30px",padding:"5px 20px"}}>-</button>
            <span>{page}</span>
            <button onClick={()=>setpage(page+1)} style={{backgroundColor:"black",color:"white",margin:"10px 30px",padding:"5px 20px"}}>+</button>
                    </div>
